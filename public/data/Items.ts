interface Item {
  name: string;
  type: string[];
  tier: number;
  location: string;
  rare?: boolean | null;
  expansion: string;
  weaponType?: string[];
}

export const items: Item[] = [
  {
    name: "Cloth",
    type: ["Armour"],
    tier: 0,
    location: "Starter",
    expansion: "Core Box",
  },
  {
    name: "Founding Stone",
    type: ["Accessory"],
    tier: 0,
    location: "Starter",
    expansion: "Core Box",
  },
  {
    name: "Rawhide Armor",
    type: ["Armour"],
    tier: 1,
    location: "Skinnery",
    expansion: "Core Box",
  },
  {
    name: "Bandages",
    type: ["Accessory"],
    tier: 1,
    location: "Skinnery",
    expansion: "Core Box",
  },
  {
    name: "Rawhide Whip",
    type: ["Weapon"],
    tier: 1,
    location: "Skinnery",
    expansion: "Core Box",
    weaponType: ["Whip"],
  },
  {
    name: "Rawhide Drum",
    type: ["Accessory"],
    tier: 1,
    location: "Skinnery",
    expansion: "Core Box",
  },
  {
    name: "Stone Noses",
    type: ["Accessory"],
    tier: 0,
    location: "Organ Grinder",
    expansion: "Core Box",
  },
  {
    name: "Fecal Salve",
    type: ["Accessory"],
    tier: 1,
    location: "Organ Grinder",
    expansion: "Core Box",
  },
  {
    name: "Monster Grease",
    type: ["Accessory"],
    tier: 1,
    location: "Organ Grinder",
    expansion: "Core Box",
  },
  {
    name: "Dried Acanthus",
    type: ["Accessory"],
    tier: 1,
    location: "Organ Grinder",
    expansion: "Core Box",
  },
  {
    name: "Lucky Charm",
    type: ["Accessory"],
    tier: 1,
    location: "Organ Grinder",
    expansion: "Core Box",
  },
  {
    name: "Monster Tooth Necklace",
    type: ["Accessory"],
    tier: 2,
    location: "Organ Grinder",
    expansion: "Core Box",
  },
  {
    name: "Gorment Armor",
    type: ["Armour"],
    tier: 1,
    location: "Gorm",
    expansion: "Gorm",
  },
  {
    name: "Regeneration Suit",
    type: ["Accessory"],
    tier: 3,
    location: "Gorm",
    expansion: "Gorm",
  },
  {
    name: "Rib Blade",
    type: ["Weapon"],
    tier: 1,
    location: "Gorm",
    expansion: "Gorm",
    weaponType: ["Grand"],
  },
  {
    name: "Knuckle Shield",
    type: ["Weapon", "Tank"],
    tier: 1,
    location: "Gorm",
    expansion: "Gorm",
    weaponType: ["Shield"],
  },
  {
    name: "Gaxe",
    type: ["Weapon"],
    tier: 1,
    location: "Gorm",
    expansion: "Gorm",
    weaponType: ["Axe"],
  },
  {
    name: "Greater Gaxe",
    type: ["Weapon"],
    tier: 2,
    location: "Gorm",
    expansion: "Gorm",
    weaponType: ["Axe"],
  },
  {
    name: "Gorn",
    type: ["Accessory"],
    tier: 2,
    location: "Gorm",
    expansion: "Gorm",
  },
  {
    name: "Riot Mace",
    type: ["Weapon"],
    tier: 3,
    location: "Gorm",
    expansion: "Gorm",
    weaponType: ["Club"],
  },
  {
    name: "Pulse Lantern",
    type: ["Accessory"],
    tier: 2,
    location: "Gorm",
    expansion: "Gorm",
  },
  {
    name: "Armor Spikes",
    type: ["Accessory"],
    tier: 2,
    location: "Gorm",
    expansion: "Gorm",
  },
  {
    name: "Acid Tooth Dagger",
    type: ["Weapon"],
    tier: 1,
    location: "Gorm",
    expansion: "Gorm",
    weaponType: ["Dagger"],
  },
  {
    name: "Black Sword",
    type: ["Weapon"],
    tier: 4,
    location: "Gorm",
    expansion: "Gorm",
    weaponType: ["Sword"],
  },
  {
    name: "Healing Potion",
    type: ["Accessory"],
    tier: 1,
    location: "Gorm",
    expansion: "Gorm",
  },
  {
    name: "Wisdom Potion",
    type: ["Accessory", "Support"],
    tier: 1,
    location: "Gorm",
    expansion: "Gorm",
  },
  {
    name: "Steadfast Potion",
    type: ["Accessory"],
    tier: 2,
    location: "Gorm",
    expansion: "Gorm",
  },
  {
    name: "Power Potion",
    type: ["Accessory"],
    tier: 2,
    location: "Gorm",
    expansion: "Gorm",
  },
  {
    name: "Life Elixir",
    type: ["Accessory"],
    tier: 3,
    location: "Gorm",
    expansion: "Gorm",
  },
  {
    name: "Bone Dagger",
    type: ["Weapon"],
    tier: 0,
    location: "Bone Smith",
    expansion: "Core Box",
    weaponType: ["Dagger"],
  },
  {
    name: "Bone Blade",
    type: ["Weapon"],
    tier: 0,
    location: "Bone Smith",
    expansion: "Core Box",
    weaponType: ["Sword"],
  },
  {
    name: "Bone Axe",
    type: ["Weapon"],
    tier: 0,
    location: "Bone Smith",
    expansion: "Core Box",
    weaponType: ["Axe"],
  },
  {
    name: "Bone Darts",
    type: ["Weapon"],
    tier: 0,
    location: "Bone Smith",
    expansion: "Core Box",
    weaponType: ["Thrown"],
  },
  {
    name: "Bone Club",
    type: ["Weapon"],
    tier: 0,
    location: "Bone Smith",
    expansion: "Core Box",
    weaponType: ["Club"],
  },
  {
    name: "Skull Helm",
    type: ["Accessory"],
    tier: 0,
    location: "Bone Smith",
    expansion: "Core Box",
  },
  {
    name: "Bone Pickaxe",
    type: ["Accessory"],
    tier: 1,
    location: "Bone Smith",
    expansion: "Core Box",
  },
  {
    name: "Bone Sickle",
    type: ["Accessory"],
    tier: 1,
    location: "Bone Smith",
    expansion: "Core Box",
  },
  {
    name: "Lantern Armor",
    type: ["Armour"],
    tier: 4,
    location: "Blacksmith",
    expansion: "Core Box",
  },
  {
    name: "Lantern Sword",
    type: ["Weapon"],
    tier: 3,
    location: "Blacksmith",
    expansion: "Core Box",
    weaponType: ["Sword"],
  },
  {
    name: "Lantern Dagger",
    type: ["Weapon"],
    tier: 3,
    location: "Blacksmith",
    expansion: "Core Box",
    weaponType: ["Dagger"],
  },
  {
    name: "Lantern Glaive",
    type: ["Weapon"],
    tier: 3,
    location: "Blacksmith",
    expansion: "Core Box",
    weaponType: ["Spear", "Axe"],
  },
  {
    name: "Dragon Slayer",
    type: ["Weapon"],
    tier: 3,
    location: "Blacksmith",
    expansion: "Core Box",
    weaponType: ["Grand"],
  },
  {
    name: "Beacon Shield",
    type: ["Tank"],
    tier: 3,
    location: "Blacksmith",
    expansion: "Core Box",
    weaponType: ["Shield"],
  },
  {
    name: "Ring Whip",
    type: ["Weapon"],
    tier: 3,
    location: "Blacksmith",
    expansion: "Core Box",
    weaponType: ["Whip"],
  },
  {
    name: "Polishing Lantern",
    type: ["Accessory"],
    tier: 3,
    location: "Blacksmith",
    expansion: "Core Box",
  },
  {
    name: "Perfect Slayer",
    type: ["Weapon"],
    tier: 4,
    location: "Blacksmith",
    expansion: "Core Box",
    weaponType: ["Grand", "Sword"],
  },
  {
    name: "Conterweight Axe",
    type: ["Weapon"],
    tier: 2,
    location: "Weapon Crafter",
    expansion: "Core Box",
    weaponType: ["Axe"],
  },
  {
    name: "Skullcap Hammer",
    type: ["Weapon"],
    tier: 2,
    location: "Weapon Crafter",
    expansion: "Core Box",
    weaponType: ["Club"],
  },
  {
    name: "Whistling Mace",
    type: ["Weapon"],
    tier: 2,
    location: "Weapon Crafter",
    expansion: "Core Box",
    weaponType: ["Club", "Whip"],
  },
  {
    name: "Scrap Sword",
    type: ["Weapon"],
    tier: 2,
    location: "Weapon Crafter",
    expansion: "Core Box",
    weaponType: ["Sword"],
  },
  {
    name: "Scrap Lantern",
    type: ["Accessory"],
    tier: 2,
    location: "Weapon Crafter",
    expansion: "Core Box",
  },
  {
    name: "Scrap Dagger",
    type: ["Weapon"],
    tier: 2,
    location: "Weapon Crafter",
    expansion: "Core Box",
    weaponType: ["Dagger"],
  },
  {
    name: "Scrap Rebar",
    type: ["Accessory"],
    tier: 2,
    location: "Weapon Crafter",
    expansion: "Core Box",
  },
  {
    name: "Scrap Bone Spear",
    type: ["Weapon"],
    tier: 2,
    location: "Weapon Crafter",
    expansion: "Core Box",
    weaponType: ["Spear"],
  },
  {
    name: "Zanbato",
    type: ["Weapon"],
    tier: 2,
    location: "Weapon Crafter",
    expansion: "Core Box",
    weaponType: ["Grand"],
  },
  {
    name: "Scrap Shield",
    type: ["Weapon", "Tank"],
    tier: 2,
    location: "Weapon Crafter",
    expansion: "Core Box",
    weaponType: ["Shield"],
  },
  {
    name: "First Aid Kit",
    type: ["Accessory"],
    tier: 2,
    location: "Barber Surgeon",
    expansion: "Core Box",
  },
  {
    name: "Scavenger Kit",
    type: ["Blacklist"],
    tier: 2,
    location: "Barber Surgeon",
    expansion: "Core Box",
  },
  {
    name: "Almanac",
    type: ["Accessory"],
    tier: 2,
    location: "Barber Surgeon",
    expansion: "Core Box",
  },
  {
    name: "Bug Trap",
    type: ["Accessory"],
    tier: 2,
    location: "Barber Surgeon",
    expansion: "Core Box",
  },
  {
    name: "Musk Bomb",
    type: ["Accessory"],
    tier: 2,
    location: "Barber Surgeon",
    expansion: "Core Box",
  },
  {
    name: "Blue Charm",
    type: ["Accessory"],
    tier: 2,
    location: "Barber Surgeon",
    expansion: "Core Box",
  },
  {
    name: "Red Charm",
    type: ["Accessory"],
    tier: 2,
    location: "Barber Surgeon",
    expansion: "Core Box",
  },
  {
    name: "Green Charm",
    type: ["Accessory"],
    tier: 2,
    location: "Barber Surgeon",
    expansion: "Core Box",
  },
  {
    name: "Screaming Armor",
    type: ["Armour"],
    tier: 1,
    location: "Stone Circle",
    expansion: "Core Box",
  },
  {
    name: "Boss Mehndi",
    type: ["Accessory"],
    tier: 1,
    location: "Stone Circle",
    expansion: "Core Box",
  },
  {
    name: "Bone Earrings",
    type: ["Accessory"],
    tier: 1,
    location: "Stone Circle",
    expansion: "Core Box",
  },
  {
    name: "Beast Knuckle",
    type: ["Weapon"],
    tier: 3,
    location: "Stone Circle",
    expansion: "Core Box",
    weaponType: ["Katar"],
  },
  {
    name: "Speed Powder",
    type: ["Accessory"],
    tier: 1,
    location: "Stone Circle",
    expansion: "Core Box",
  },
  {
    name: "Brain Mint",
    type: ["Accessory"],
    tier: 1,
    location: "Stone Circle",
    expansion: "Core Box",
  },
  {
    name: "Elder Earrings",
    type: ["Accessory"],
    tier: 1,
    location: "Stone Circle",
    expansion: "Core Box",
  },
  {
    name: "Blood Paint",
    type: ["Accessory"],
    tier: 2,
    location: "Stone Circle",
    expansion: "Core Box",
  },
  {
    name: "Lance Of Longinus",
    type: ["Weapon"],
    tier: 4,
    location: "Stone Circle",
    expansion: "Core Box",
    weaponType: ["Spear"],
  },
  {
    name: "Dragon Armor",
    type: ["Armour"],
    tier: 3,
    location: "Dragon Armoury",
    expansion: "Dragon King",
  },
  {
    name: "Dragon Chakram",
    type: ["Weapon"],
    tier: 2,
    location: "Dragon Armoury",
    expansion: "Dragon King",
    weaponType: ["Thrown"],
  },
  {
    name: "Blast Shield",
    type: ["Weapon", "Tank"],
    tier: 3,
    location: "Dragon Armoury",
    expansion: "Dragon King",
    weaponType: ["Shield"],
  },
  {
    name: "Dragon Bite Bolt",
    type: ["Accessory"],
    tier: 2,
    location: "Dragon Armoury",
    expansion: "Dragon King",
  },
  {
    name: "Shielded Quiver",
    type: ["Accessory"],
    tier: 3,
    location: "Dragon Armoury",
    expansion: "Dragon King",
  },
  {
    name: "Talon Knife",
    type: ["Weapon"],
    tier: 2,
    location: "Dragon Armoury",
    expansion: "Dragon King",
    weaponType: ["Katar"],
  },
  {
    name: "Blast Sword",
    type: ["Weapon"],
    tier: 2,
    location: "Dragon Armoury",
    expansion: "Dragon King",
    weaponType: ["Sword"],
  },
  {
    name: "Nuclear Knife",
    type: ["Weapon"],
    tier: 3,
    location: "Dragon Armoury",
    expansion: "Dragon King",
    weaponType: ["Dagger"],
  },
  {
    name: "Nuclear Scythe",
    type: ["Weapon"],
    tier: 3,
    location: "Dragon Armoury",
    expansion: "Dragon King",
    weaponType: ["Scythe"],
  },
  {
    name: "Red Core",
    type: ["Accessory"],
    tier: 3,
    location: "Dragon Armoury",
    expansion: "Dragon King",
  },
  {
    name: "Blue Core",
    type: ["Accessory"],
    tier: 3,
    location: "Dragon Armoury",
    expansion: "Dragon King",
  },
  {
    name: "Rolling Armor (non calcified)",
    type: ["Armour"],
    tier: 3,
    location: "Dung Beetle Knight Gear",
    expansion: "Dung Beetle Knight",
  },
  {
    name: "Rolling Armor (calcified)",
    type: ["Armour"],
    tier: 4,
    location: "Dung Beetle Knight Gear",
    expansion: "Dung Beetle Knight",
  },
  {
    name: "DBK Errant Badge",
    type: ["Accessory"],
    tier: 3,
    location: "Dung Beetle Knight Gear",
    expansion: "Dung Beetle Knight",
  },
  {
    name: "Rainbow Wing Belt",
    type: ["Accessory"],
    tier: 3,
    location: "Dung Beetle Knight Gear",
    expansion: "Dung Beetle Knight",
  },
  {
    name: "Digging Claws",
    type: ["Weapon"],
    tier: 3,
    location: "Dung Beetle Knight Gear",
    expansion: "Dung Beetle Knight",
    weaponType: ["Katar"],
  },
  {
    name: "Seasoned Monster Meat",
    type: ["Accessory"],
    tier: 3,
    location: "Dung Beetle Knight Gear",
    expansion: "Dung Beetle Knight",
  },
  {
    name: "Century Shoulder Pads",
    type: ["Accessory"],
    tier: 3,
    location: "Dung Beetle Knight Gear",
    expansion: "Dung Beetle Knight",
  },
  {
    name: "Scarab Circlet",
    type: ["Accessory"],
    tier: 3,
    location: "Dung Beetle Knight Gear",
    expansion: "Dung Beetle Knight",
  },
  {
    name: "Century Greaves",
    type: ["Accessory"],
    tier: 3,
    location: "Dung Beetle Knight Gear",
    expansion: "Dung Beetle Knight",
  },
  {
    name: "Elastic Harness",
    type: ["Accessory"],
    tier: 3,
    location: "Dung Beetle Knight Gear",
    expansion: "Dung Beetle Knight",
  },
  {
    name: "The Beetle Bomb",
    type: ["Accessory"],
    tier: 4,
    location: "Dung Beetle Knight Gear",
    expansion: "Dung Beetle Knight",
  },
  {
    name: "Calcified Juggernaut Blade",
    type: ["Weapon"],
    tier: 3,
    location: "Dung Beetle Knight Gear",
    expansion: "Dung Beetle Knight",
    weaponType: ["Grand"],
  },
  {
    name: "Calcified Zanbato",
    type: ["Weapon"],
    tier: 3,
    location: "Dung Beetle Knight Gear",
    expansion: "Dung Beetle Knight",
    weaponType: ["Grand"],
  },
  {
    name: "Calcified Digging Claw",
    type: ["Weapon"],
    tier: 3,
    location: "Dung Beetle Knight Gear",
    expansion: "Dung Beetle Knight",
    weaponType: ["Katar"],
  },
  {
    name: "Calcified Greaves",
    type: ["Accessory"],
    tier: 3,
    location: "Dung Beetle Knight Gear",
    expansion: "Dung Beetle Knight",
  },
  {
    name: "Calcified Shoulder Pads",
    type: ["Accessory"],
    tier: 3,
    location: "Dung Beetle Knight Gear",
    expansion: "Dung Beetle Knight",
  },
  {
    name: "Leather Armor",
    type: ["Armour"],
    tier: 2,
    location: "Leather Worker",
    expansion: "Core Box",
  },
  {
    name: "Round Leather Shield",
    type: ["Tank"],
    tier: 2,
    location: "Leather Worker",
    expansion: "Core Box",
  },
  {
    name: "Hunter Whip",
    type: ["Weapon"],
    tier: 2,
    location: "Leather Worker",
    expansion: "Core Box",
    weaponType: ["Whip"],
  },
  {
    name: "Phoenix Armor",
    type: ["Armour"],
    tier: 3,
    location: "Plumery",
    expansion: "Core Box",
  },
  {
    name: "Feather Shield",
    type: ["Weapon", "Tank"],
    tier: 2,
    location: "Plumery",
    expansion: "Core Box",
    weaponType: ["Shield"],
  },
  {
    name: "Feather Mantle",
    type: ["Accessory"],
    tier: 2,
    location: "Plumery",
    expansion: "Core Box",
  },
  {
    name: "Bloom Sphere",
    type: ["Accessory"],
    tier: 2,
    location: "Plumery",
    expansion: "Core Box",
  },
  {
    name: "Bird Bread",
    type: ["Accessory"],
    tier: 2,
    location: "Plumery",
    expansion: "Core Box",
  },
  {
    name: "Sonic Tomahawk",
    type: ["Weapon"],
    tier: 3,
    location: "Plumery",
    expansion: "Core Box",
    weaponType: ["Axe"],
  },
  {
    name: "Hours Ring",
    type: ["Blacklist"],
    tier: 4,
    location: "Plumery",
    expansion: "Core Box",
  },
  {
    name: "Hollow Sword",
    type: ["Weapon"],
    tier: 2,
    location: "Plumery",
    expansion: "Core Box",
    weaponType: ["Sword", "Dagger"],
  },
  {
    name: "Crest Crown",
    type: ["Accessory", "Support"],
    tier: 4,
    location: "Plumery",
    expansion: "Core Box",
  },
  {
    name: "Arc Bow",
    type: ["Weapon"],
    tier: 2,
    location: "Plumery",
    expansion: "Core Box",
    weaponType: ["Bow"],
  },
  {
    name: "Hollowpoint Arrow",
    type: ["Accessory"],
    tier: 2,
    location: "Plumery",
    expansion: "Core Box",
  },
  {
    name: "Finger Of God",
    type: ["Weapon"],
    tier: 2,
    location: "Plumery",
    expansion: "Core Box",
    weaponType: ["Spear"],
  },
  {
    name: "Blood Sheath",
    type: ["Accessory"],
    tier: 3,
    location: "Plumery",
    expansion: "Core Box",
  },
  {
    name: "Rainbow Katana",
    type: ["Weapon"],
    tier: 3,
    location: "Plumery",
    expansion: "Core Box",
    weaponType: ["Katana"],
  },
  {
    name: "White Lion Mask",
    type: ["Accessory"],
    tier: 4,
    location: "Mask Maker",
    expansion: "Core Box",
  },
  {
    name: "Antelope Mask",
    type: ["Accessory"],
    tier: 4,
    location: "Mask Maker",
    expansion: "Core Box",
  },
  {
    name: "Phoenix Mask",
    type: ["Accessory"],
    tier: 4,
    location: "Mask Maker",
    expansion: "Core Box",
  },
  {
    name: "Death Mask",
    type: ["Accessory"],
    tier: 4,
    location: "Mask Maker",
    expansion: "Core Box",
  },
  {
    name: "Man Mask",
    type: ["Accessory"],
    tier: 4,
    location: "Mask Maker",
    expansion: "Core Box",
  },
  {
    name: "God Mask",
    type: ["Accessory"],
    tier: 4,
    location: "Mask Maker",
    expansion: "Core Box",
  },
  {
    name: "Silk Armor",
    type: ["Armour"],
    tier: 2,
    location: "Silk Mill",
    expansion: "Spidicules",
  },
  {
    name: "Amber Poleaxe",
    type: ["Weapon"],
    tier: 2,
    location: "Silk Mill",
    expansion: "Spidicules",
    weaponType: ["Axe", "Spear"],
  },
  {
    name: "Body Suit",
    type: ["Accessory"],
    tier: 2,
    location: "Silk Mill",
    expansion: "Spidicules",
  },
  {
    name: "Throwing Knife",
    type: ["Weapon"],
    tier: 1,
    location: "Silk Mill",
    expansion: "Spidicules",
    weaponType: ["Thrown"],
  },
  {
    name: "Silk Bomb",
    type: ["Accessory"],
    tier: 2,
    location: "Silk Mill",
    expansion: "Spidicules",
  },
  {
    name: "Silk Whip",
    type: ["Weapon"],
    tier: 2,
    location: "Silk Mill",
    expansion: "Spidicules",
    weaponType: ["Whip"],
  },
  {
    name: "Hooded Scrap Katar",
    type: ["Weapon"],
    tier: 1,
    location: "Silk Mill",
    expansion: "Spidicules",
    weaponType: ["Katar"],
  },
  {
    name: "Amber Edge",
    type: ["Weapon"],
    tier: 1,
    location: "Silk Mill",
    expansion: "Spidicules",
  },
  {
    name: "Red Ring",
    type: ["Accessory"],
    tier: 3,
    location: "Silk Mill",
    expansion: "Spidicules",
  },
  {
    name: "Blue Ring",
    type: ["Accessory"],
    tier: 3,
    location: "Silk Mill",
    expansion: "Spidicules",
  },
  {
    name: "Green Ring",
    type: ["Accessory"],
    tier: 3,
    location: "Silk Mill",
    expansion: "Spidicules",
  },
  {
    name: "White Lion Armor",
    type: ["Armour"],
    tier: 1,
    location: "Catarium",
    expansion: "Core Box",
  },
  {
    name: "Lion Beast Katar",
    type: ["Weapon"],
    tier: 1,
    location: "Catarium",
    expansion: "Core Box",
    weaponType: ["Katar"],
  },
  {
    name: "Cat Eye Circlet",
    type: ["Accessory", "Support"],
    tier: 1,
    location: "Catarium",
    expansion: "Core Box",
  },
  {
    name: "Claw Head Arrow",
    type: ["Accessory"],
    tier: 1,
    location: "Catarium",
    expansion: "Core Box",
  },
  {
    name: "Whisker Harp",
    type: ["Accessory"],
    tier: 1,
    location: "Catarium",
    expansion: "Core Box",
  },
  {
    name: "King Spear",
    type: ["Weapon"],
    tier: 1,
    location: "Catarium",
    expansion: "Core Box",
    weaponType: ["Spear"],
  },
  {
    name: "Lion Headdress",
    type: ["Accessory"],
    tier: 1,
    location: "Catarium",
    expansion: "Core Box",
  },
  {
    name: "Cat Gut Bow",
    type: ["Weapon"],
    tier: 1,
    location: "Catarium",
    expansion: "Core Box",
    weaponType: ["Bow"],
  },
  {
    name: "Lion Skin Cloak",
    type: ["Accessory"],
    tier: 1,
    location: "Catarium",
    expansion: "Core Box",
  },
  {
    name: "Cat Fang Knife",
    type: ["Weapon"],
    tier: 3,
    location: "Catarium",
    expansion: "Core Box",
    weaponType: ["Dagger"],
  },
  {
    name: "Frenzy Drink",
    type: ["Accessory"],
    tier: 1,
    location: "Catarium",
    expansion: "Core Box",
  },
  {
    name: "Cycloid Scale Armor",
    type: ["Armour"],
    tier: 2,
    location: "Skyreef Sanctuary",
    expansion: "Sunstalker",
  },
  {
    name: "Sunspot Dart",
    type: ["Weapon"],
    tier: 2,
    location: "Skyreef Sanctuary",
    expansion: "Sunstalker",
    weaponType: ["Thrown"],
  },
  {
    name: "Sunspot Lantern",
    type: ["Accessory"],
    tier: 2,
    location: "Skyreef Sanctuary",
    expansion: "Sunstalker",
  },
  {
    name: "Sunshark Bow",
    type: ["Weapon"],
    tier: 2,
    location: "Skyreef Sanctuary",
    expansion: "Sunstalker",
    weaponType: ["Bow"],
  },
  {
    name: "Quiver And Sunstring",
    type: ["Accessory"],
    tier: 2,
    location: "Skyreef Sanctuary",
    expansion: "Sunstalker",
  },
  {
    name: "Sunshark Arrows",
    type: ["Accessory"],
    tier: 2,
    location: "Skyreef Sanctuary",
    expansion: "Sunstalker",
  },
  {
    name: "Shadow Saliva Shawl",
    type: ["Accessory"],
    tier: 2,
    location: "Skyreef Sanctuary",
    expansion: "Sunstalker",
  },
  {
    name: "Denticle Axe",
    type: ["Weapon"],
    tier: 3,
    location: "Skyreef Sanctuary",
    expansion: "Sunstalker",
    weaponType: ["Axe"],
  },
  {
    name: "Sun Lure And Hook",
    type: ["Accessory"],
    tier: 2,
    location: "Skyreef Sanctuary",
    expansion: "Sunstalker",
  },
  {
    name: "Skleaver",
    type: ["Weapon"],
    tier: 3,
    location: "Skyreef Sanctuary",
    expansion: "Sunstalker",
    weaponType: ["Grand"],
  },
  {
    name: "Sky Harpoon",
    type: ["Weapon"],
    tier: 2,
    location: "Skyreef Sanctuary",
    expansion: "Sunstalker",
    weaponType: ["Spear", "Thrown"],
  },
  {
    name: "Ink Sword",
    type: ["Weapon"],
    tier: 4,
    location: "Skyreef Sanctuary",
    expansion: "Sunstalker",
    weaponType: ["Sword"],
  },
  {
    name: "Ink Blood Bow",
    type: ["Weapon"],
    tier: 4,
    location: "Skyreef Sanctuary",
    expansion: "Sunstalker",
    weaponType: ["Bow"],
  },
  {
    name: "Sun Vestments",
    type: ["Accessory"],
    tier: 2,
    location: "Rare Gear",
    expansion: "Sunstalker",
    rare: true,
  },
  {
    name: "Sunring Bow",
    type: ["Weapon"],
    tier: 3,
    location: "Rare Gear",
    expansion: "Sunstalker",
    rare: true,
    weaponType: ["Bow"],
  },
  {
    name: "Apostle Crown",
    type: ["Accessory", "Support"],
    tier: 4,
    location: "Rare Gear",
    expansion: "Sunstalker",
    rare: true,
  },
  {
    name: "Prism Mace",
    type: ["Weapon"],
    tier: 4,
    location: "Rare Gear",
    expansion: "Sunstalker",
    rare: true,
    weaponType: ["Club"],
  },
  {
    name: "Vespertine Bow",
    type: ["Weapon"],
    tier: 1,
    location: "Flowerknight Gear",
    expansion: "Flowerknight",
    weaponType: ["Bow"],
  },
  {
    name: "Vespertine Foil",
    type: ["Weapon"],
    tier: 1,
    location: "Flowerknight Gear",
    expansion: "Flowerknight",
    weaponType: ["Sword"],
  },
  {
    name: "Vespertine Arrow",
    type: ["Accessory"],
    tier: 1,
    location: "Flowerknight Gear",
    expansion: "Flowerknight",
  },
  {
    name: "Vespertine Cello",
    type: ["Accessory"],
    tier: 1,
    location: "Flowerknight Gear",
    expansion: "Flowerknight",
  },
  {
    name: "Flower Knight Badge",
    type: ["Accessory"],
    tier: 1,
    location: "Flowerknight Gear",
    expansion: "Flowerknight",
  },
  {
    name: "SFA: Acanthus Doctor, 3 Satchels, 3 Fresh Acanthus",
    type: ["Armour"],
    tier: 3,
    location: "Flowerknight Gear",
    expansion: "Flowerknight",
  },
  {
    name: "Satchel, 1 Fresh Acanthus",
    type: ["Accessory"],
    tier: 2,
    location: "Flowerknight Gear",
    expansion: "Flowerknight",
  },
  {
    name: "Gloom Hammer",
    type: ["Weapon"],
    tier: 3,
    location: "Slenderman Gear",
    expansion: "Slenderman",
    weaponType: ["Club"],
  },
  {
    name: "Gloom Katana and Gloom Sheath",
    type: ["Weapon"],
    tier: 4,
    location: "Slenderman Gear",
    expansion: "Slenderman",
    weaponType: ["Katana"],
  },
  {
    name: "Gloom-Coated Arrow",
    type: ["Accessory"],
    tier: 2,
    location: "Slenderman Gear",
    expansion: "Slenderman",
  },
  {
    name: "Dark Water Vial",
    type: ["Accessory"],
    tier: 2,
    location: "Slenderman Gear",
    expansion: "Slenderman",
  },
  {
    name: "Gloom Cream",
    type: ["Blacklist"],
    tier: 2,
    location: "Slenderman Gear",
    expansion: "Slenderman",
  },
  {
    name: "Gloom Mehndi",
    type: ["Accessory"],
    tier: 2,
    location: "Slenderman Gear",
    expansion: "Slenderman",
  },
  {
    name: "Raptor-Worm Collar",
    type: ["Accessory"],
    tier: 3,
    location: "Slenderman Gear",
    expansion: "Slenderman",
  },
  {
    name: "Slender Ovule",
    type: ["Accessory"],
    tier: 3,
    location: "Slenderman Gear",
    expansion: "Slenderman",
  },
  {
    name: "Gloom Bracelets",
    type: ["Accessory"],
    tier: 2,
    location: "Slenderman Gear",
    expansion: "Slenderman",
  },
  {
    name: "Oxidized Beacon Shield",
    type: ["Tank"],
    tier: 4,
    location: "Exhausted Lantern Hoard",
    expansion: "Core Box",
  },
  {
    name: "Oxidized Lantern Dagger",
    type: ["Weapon"],
    tier: 4,
    location: "Exhausted Lantern Hoard",
    expansion: "Core Box",
    weaponType: ["Dagger"],
  },
  {
    name: "Oxidized Lantern Glaive",
    type: ["Weapon"],
    tier: 4,
    location: "Exhausted Lantern Hoard",
    expansion: "Core Box",
    weaponType: ["Spear", "Axe"],
  },
  {
    name: "Oxidized Lantern Sword",
    type: ["Weapon"],
    tier: 4,
    location: "Exhausted Lantern Hoard",
    expansion: "Core Box",
    weaponType: ["Sword"],
  },
  {
    name: "Oxidized Ring Whip",
    type: ["Weapon"],
    tier: 4,
    location: "Exhausted Lantern Hoard",
    expansion: "Core Box",
    weaponType: ["Whip"],
  },
  {
    name: "Oxidized Lantern Helm (with Lantern Armor)",
    type: ["Armour"],
    tier: 4,
    location: "Exhausted Lantern Hoard",
    expansion: "Core Box",
  },
  {
    name: "Maul Scab",
    type: ["Weapon"],
    tier: 2,
    location: "Pattern(Pariah)",
    expansion: "Pariah",
    weaponType: ["Club"],
  },
  {
    name: "Knife Scab",
    type: ["Weapon"],
    tier: 2,
    location: "Pattern(Pariah)",
    expansion: "Pariah",
    weaponType: ["Dagger"],
  },
  {
    name: "Halberd Scab",
    type: ["Weapon"],
    tier: 2,
    location: "Pattern(Pariah)",
    expansion: "Pariah",
    weaponType: ["Spear"],
  },
  {
    name: "Sword Scab",
    type: ["Accessory"],
    tier: 2,
    location: "Pattern(Pariah)",
    expansion: "Pariah",
  },
  {
    name: "Bow Scab",
    type: ["Weapon"],
    tier: 2,
    location: "Pattern(Pariah)",
    expansion: "Pariah",
    weaponType: ["Bow"],
  },
  {
    name: "Marchioness Armor",
    type: ["Armour"],
    tier: 3,
    location: "Pattern(Black Knight)",
    expansion: "Black Knight",
  },
  {
    name: "Count Armor",
    type: ["Armour"],
    tier: 3,
    location: "Pattern(Black Knight)",
    expansion: "Black Knight",
  },
  {
    name: "Earl Armor",
    type: ["Armour"],
    tier: 3,
    location: "Pattern(Black Knight)",
    expansion: "Black Knight",
  },
  {
    name: "Squire Armor",
    type: ["Armour"],
    tier: 1,
    location: "Squire Gear Grid(Black Knight)",
    expansion: "Black Knight",
    rare: true,
  },
  {
    name: "Black & Red Armor and Deprivation Skinsuit",
    type: ["Armour"],
    tier: 4,
    location: "Sanguine Extrusion",
    expansion: "Red Witches",
  },
  {
    name: "Black & Red Armor",
    type: ["Armour"],
    tier: 3,
    location: "Sanguine Extrusion",
    expansion: "Red Witches",
  },
  {
    name: "Red Cloak",
    type: ["Accessory"],
    tier: 2,
    location: "Sanguine Extrusion",
    expansion: "Red Witches",
  },
  {
    name: "Hyperelastic Bow",
    type: ["Weapon"],
    tier: 2,
    location: "Sanguine Extrusion",
    expansion: "Red Witches",
    weaponType: ["Bow"],
  },
  {
    name: "Hyperelastic Sword",
    type: ["Weapon"],
    tier: 2,
    location: "Sanguine Extrusion",
    expansion: "Red Witches",
    weaponType: ["Sword"],
  },
  {
    name: "Hyperelastic Shield",
    type: ["Tank"],
    tier: 2,
    location: "Sanguine Extrusion",
    expansion: "Red Witches",
  },
  {
    name: "Warlord Armor",
    type: ["Armour"],
    tier: 3,
    location: "Mixed (Lion Knight)",
    rare: true,
    expansion: "Lion Knight",
  },
  {
    name: "Dancer Armor",
    type: ["Armour"],
    tier: 2,
    location: "Mixed (Lion Knight)",
    rare: true,
    expansion: "Lion Knight",
  },
  {
    name: "Brawler Armor",
    type: ["Armour"],
    tier: 2,
    location: "Mixed (Lion Knight)",
    rare: true,
    expansion: "Lion Knight",
  },
  {
    name: "Lion God Statue",
    type: ["Accessory"],
    tier: 3,
    location: "Rare Gear (Lion God)",
    rare: true,
    expansion: "Lion God",
  },
  {
    name: "Bone Witch Mehndi",
    type: ["Accessory"],
    tier: 3,
    location: "Rare Gear (Lion God)",
    rare: true,
    expansion: "Lion God",
  },
  {
    name: "Butchers Blood",
    type: ["Accessory"],
    tier: 3,
    location: "Rare Gear (Lion God)",
    rare: true,
    expansion: "Lion God",
  },
  {
    name: "Golden Plate",
    type: ["Accessory"],
    tier: 3,
    location: "Rare Gear (Lion God)",
    rare: true,
    expansion: "Lion God",
  },
  {
    name: "Ancient Lion Claws",
    type: ["Accessory"],
    tier: 3,
    location: "Rare Gear (Lion God)",
    rare: true,
    expansion: "Lion God",
  },
  {
    name: "Glyph Of Solitude",
    type: ["Accessory"],
    tier: 3,
    location: "Rare Gear (Lion God)",
    rare: true,
    expansion: "Lion God",
  },
  {
    name: "Death Mehndi",
    type: ["Accessory"],
    tier: 3,
    location: "Rare Gear (Lion God)",
    rare: true,
    expansion: "Lion God",
  },
  {
    name: "Necromancer's Eye",
    type: ["Accessory", "Support"],
    tier: 3,
    location: "Rare Gear (Lion God)",
    rare: true,
    expansion: "Lion God",
  },
  {
    name: "Lantern Mehndi",
    type: ["Accessory"],
    tier: 3,
    location: "Rare Gear (Lion God)",
    rare: true,
    expansion: "Lion God",
  },
  {
    name: "Singing Armor",
    type: ["Armour"],
    tier: 1,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Chorusseum",
  },
  {
    name: "Hamfluter",
    type: ["Weapon"],
    tier: 1,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Chorusseum",
  },
  {
    name: "Peace Dagger",
    type: ["Weapon"],
    tier: 1,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Chorusseum",
  },
  {
    name: "Pipa",
    type: ["Accessory"],
    tier: 1,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Chorusseum",
  },
  {
    name: "Razor Cymbals",
    type: ["Accessory"],
    tier: 1,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Chorusseum",
  },
  {
    name: "Spear Of Life",
    type: ["Weapon"],
    tier: 1,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Chorusseum",
  },
  {
    name: "Energy Drum",
    type: ["Accessory"],
    tier: 1,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Chorusseum",
  },
  {
    name: "Harpy Harp",
    type: ["Accessory"],
    tier: 1,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Chorusseum",
  },
  {
    name: "Singing Heart",
    type: ["Accessory"],
    tier: 1,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Chorusseum",
  },
  {
    name: "Spinning Sword",
    type: ["Weapon"],
    tier: 1,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Chorusseum",
  },
  {
    name: "Crimson Armor",
    type: ["Armour"],
    tier: 1,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Crimson Crockery",
  },
  {
    name: "Bloodglass Dagger",
    type: ["Weapon"],
    tier: 1,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Crimson Crockery",
  },
  {
    name: "Finger Darts",
    type: ["Weapon"],
    tier: 1,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Crimson Crockery",
  },
  {
    name: "Crocbone Hammer",
    type: ["Weapon"],
    tier: 1,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Crimson Crockery",
  },
  {
    name: "Bloodglass Cleaver",
    type: ["Weapon"],
    tier: 1,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Crimson Crockery",
  },
  {
    name: "Bloodglass Longsword",
    type: ["Weapon"],
    tier: 1,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Crimson Crockery",
  },
  {
    name: "Bloodglass Katar",
    type: ["Weapon"],
    tier: 1,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Crimson Crockery",
  },
  {
    name: "Blood Compass Lantern",
    type: ["Accessory"],
    tier: 1,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Crimson Crockery",
  },
  {
    name: "Bloodglass Saw",
    type: ["Weapon"],
    tier: 1,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Crimson Crockery",
  },
  {
    name: "Crocodileyes",
    type: ["Accessory"],
    tier: 1,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Crimson Crockery",
  },
  {
    name: "Crimson Pearls",
    type: ["Accessory"],
    tier: 1,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Crimson Crockery",
  },
  {
    name: "Crimson Bow",
    type: ["Weapon"],
    tier: 2,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Crimson Crockery",
  },
  {
    name: "Giggling Scythe",
    type: ["Weapon"],
    tier: 3,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Crimson Crockery",
  },
  {
    name: "Fortune Lantern Sword",
    type: ["Weapon"],
    tier: 4,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Gaming Gazebo",
  },
  {
    name: "Fortune Lantern Dagger",
    type: ["Weapon"],
    tier: 4,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Gaming Gazebo",
  },
  {
    name: "Fortune Lantern Helm, Fortune Lantern Cuirass with Lantern Armor",
    type: ["Armour"],
    tier: 4,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Gaming Gazebo",
  },
  {
    name: "Gamblers Lantern",
    type: ["Accessory"],
    tier: 4,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Gaming Gazebo",
  },
  {
    name: "Deathking Armour",
    type: ["Armour"],
    tier: 4,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Kingsmith",
  },
  {
    name: "Kingslayer",
    type: ["Weapon"],
    tier: 4,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Kingsmith",
  },
  {
    name: "Grand Kingslayer",
    type: ["Weapon"],
    tier: 4,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Kingsmith",
  },
  {
    name: "Adjudicator",
    type: ["Weapon"],
    tier: 4,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Kingsmith",
  },
  {
    name: "Usurper",
    type: ["Weapon"],
    tier: 4,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Kingsmith",
  },
  {
    name: "Assassinator",
    type: ["Weapon"],
    tier: 4,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Kingsmith",
  },
  {
    name: "Royal Shield",
    type: ["Weapon", "Tank"],
    tier: 4,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Kingsmith",
  },
  {
    name: "Royal Medalion",
    type: ["Accessory"],
    tier: 4,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Kingsmith",
  },
  {
    name: "Royal Lantern",
    type: ["Accessory"],
    tier: 4,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Kingsmith",
  },
  {
    name: "Symbol Katana`",
    type: ["Weapon"],
    tier: 5,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Symbols (The King)",
  },
  {
    name: "Symbol Sword",
    type: ["Weapon"],
    tier: 5,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Symbols (The King)",
  },
  {
    name: "Symbol Whip",
    type: ["Weapon"],
    tier: 5,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Symbols (The King)",
  },
  {
    name: "Symbol Spear",
    type: ["Weapon"],
    tier: 5,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Symbols (The King)",
  },
  {
    name: "Symbol Club",
    type: ["Weapon"],
    tier: 5,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Symbols (The King)",
  },
  {
    name: "Stone Face Cloak",
    type: ["Scout"],
    tier: 2,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Outskirts",
  },
  {
    name: "Scout's Lantern",
    type: ["Blacklist"],
    tier: 1,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Outskirts",
  },
  {
    name: "Vermin Bellyboots",
    type: ["Scout"],
    tier: 2,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Outskirts",
  },
  {
    name: "Hunting Salve",
    type: ["Accessory"],
    tier: 2,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Outskirts",
  },
  {
    name: "Grimjaw",
    type: ["Scout"],
    tier: 3,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Outskirts",
  },
  {
    name: "Blood Pack",
    type: ["Blacklist"],
    tier: 2,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Outskirts",
  },
  {
    name: "Disguise Kit",
    type: ["Accessory"],
    tier: 1,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Outskirts",
  },
  {
    name: "Scout Whistle",
    type: ["Accessory"],
    tier: 2,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Outskirts",
  },
  {
    name: "Tablet (or Grimjaw if no tablet)",
    type: ["Scout"],
    tier: 4,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Outskirts",
  },
  {
    name: "White Lion Bread Box",
    type: ["Accessory"],
    tier: 4,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Meal",
  },
  {
    name: "Time Milk",
    type: ["Accessory"],
    tier: 4,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Meal",
  },
  {
    name: "Supreme Stew",
    type: ["Accessory"],
    tier: 4,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Meal",
  },
  {
    name: "Smog Soup",
    type: ["Accessory"],
    tier: 4,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Meal",
  },
  {
    name: "Fetid Frog Feta",
    type: ["Accessory"],
    tier: 4,
    rare: false,
    expansion: "Frogdog",
    location: "Meal",
  },
  {
    name: "King Lantern Sausage",
    type: ["Accessory"],
    tier: 4,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Meal",
  },
  {
    name: "Fatty Crimson Goulash",
    type: ["Accessory"],
    tier: 4,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Meal",
  },
  {
    name: "Seared Beast Skewer",
    type: ["Accessory"],
    tier: 4,
    rare: false,
    expansion: "Gamblers Chest",
    location: "Meal",
  },
  {
    name: "Frogdog Armor",
    type: ["Armour"],
    tier: 1,
    rare: false,
    expansion: "Frogdog",
    location: "Froskrafter",
  },
  {
    name: "Vertebrayonet",
    type: ["Weapon"],
    tier: 1,
    rare: false,
    expansion: "Frogdog",
    location: "Froskrafter",
  },
  {
    name: "Long Club",
    type: ["Weapon"],
    tier: 1,
    rare: false,
    expansion: "Frogdog",
    location: "Froskrafter",
  },
  {
    name: "Crest Axe",
    type: ["Weapon"],
    tier: 1,
    rare: false,
    expansion: "Frogdog",
    location: "Froskrafter",
  },
  {
    name: "Grappleclaw",
    type: ["Weapon"],
    tier: 1,
    rare: false,
    expansion: "Frogdog",
    location: "Froskrafter",
  },
  {
    name: "Spiked Buckler",
    type: ["Tank"],
    tier: 1,
    rare: false,
    expansion: "Frogdog",
    location: "Froskrafter",
  },
  {
    name: "Acid Bomb",
    type: ["Accessory"],
    tier: 1,
    rare: false,
    expansion: "Frogdog",
    location: "Froskrafter",
  },
  {
    name: "Frogdoggles",
    type: ["Accessory"],
    tier: 1,
    rare: false,
    expansion: "Frogdog",
    location: "Froskrafter",
  },
  {
    name: "Turbo Tonic",
    type: ["Accessory"],
    tier: 1,
    rare: false,
    expansion: "Frogdog",
    location: "Froskrafter",
  },
  {
    name: "Vigor Capsule",
    type: ["Accessory"],
    tier: 1,
    rare: false,
    expansion: "Frogdog",
    location: "Froskrafter",
  },
  {
    name: "Lure Lantern",
    type: ["Accessory"],
    tier: 2,
    rare: false,
    expansion: "Frogdog",
    location: "Froskrafter",
  },
  {
    name: "Frogwhistle",
    type: ["Accessory"],
    tier: 2,
    rare: false,
    expansion: "Frogdog",
    location: "Froskrafter",
  },
  {
    name: "Bullfrog Armor",
    type: ["Armour"],
    tier: 4,
    rare: false,
    expansion: "Frogdog",
    location: "Tuskworks",
  },
  {
    name: "Bullfrog Halberd",
    type: ["Weapon"],
    tier: 4,
    rare: false,
    expansion: "Frogdog",
    location: "Tuskworks",
  },
  {
    name: "Bullfrog Mace",
    type: ["Weapon"],
    tier: 4,
    rare: false,
    expansion: "Frogdog",
    location: "Tuskworks",
  },
  {
    name: "Bullfrog Scimitar",
    type: ["Weapon"],
    tier: 4,
    rare: false,
    expansion: "Frogdog",
    location: "Tuskworks",
  },
  {
    name: "Bullcharge Shield",
    type: ["Tank"],
    tier: 4,
    rare: false,
    expansion: "Frogdog",
    location: "Tuskworks",
  },
  {
    name: "Heavyfrog Lantern",
    type: ["Accessory"],
    tier: 4,
    rare: false,
    expansion: "Frogdog",
    location: "Tuskworks",
  },
  {
    name: "Bullfrog Bauble",
    type: ["Accessory"],
    tier: 4,
    rare: false,
    expansion: "Frogdog",
    location: "Tuskworks",
  },
  {
    name: "Butcher Cleaver",
    type: ["Weapon"],
    tier: 2,
    location: "Rare Gear",
    rare: true,
    expansion: "Core Box",
  },
  {
    name: "Forsaker Mask",
    type: ["Accessory"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Core Box",
  },
  {
    name: "Lantern Halberd",
    type: ["Weapon"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Core Box",
    weaponType: ["Axe", "Spear"],
  },
  {
    name: "Muramasa",
    type: ["Weapon"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Core Box",
    weaponType: ["Katana"],
  },
  {
    name: "Thunder Maul",
    type: ["Weapon"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Core Box",
    weaponType: ["Club"],
  },
  {
    name: "Steel Sword",
    type: ["Weapon"],
    tier: 2,
    location: "Rare Gear",
    rare: true,
    expansion: "Core Box",
    weaponType: ["Sword"],
  },
  {
    name: "Steel Shield",
    type: ["Tank"],
    tier: 2,
    location: "Rare Gear",
    rare: true,
    expansion: "Core Box",
  },
  {
    name: "Husk Of Destiny",
    type: ["Accessory"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Dragon King",
  },
  {
    name: "Celestial Spear",
    type: ["Weapon"],
    tier: 2,
    location: "Rare Gear",
    rare: true,
    expansion: "Dragon King",
    weaponType: ["Spear"],
  },
  {
    name: "Hazmat Shield",
    type: ["Tank", "Weapon"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Dragon King",
    weaponType: ["Shield"],
  },
  {
    name: "Regal Edge",
    type: ["Weapon"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Dragon King",
    weaponType: ["Sword"],
  },
  {
    name: "Dragon Vestments",
    type: ["Accessory"],
    tier: 2,
    location: "Rare Gear",
    rare: true,
    expansion: "Dragon King",
  },
  {
    name: "Hideous Disguise",
    type: ["Accessory"],
    tier: 2,
    location: "Rare Gear",
    rare: true,
    expansion: "Lion Knight",
  },
  {
    name: "Lion Knight's Left Claw",
    type: ["Weapon"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Lion Knight",
    weaponType: ["Katar"],
  },
  {
    name: "Lion Knight's Right Claw",
    type: ["Weapon"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Lion Knight",
    weaponType: ["Katar"],
  },
  {
    name: "Both Lion Knight Claws",
    type: ["Weapon"],
    tier: 4,
    location: "Rare Gear",
    rare: true,
    expansion: "Lion Knight",
    weaponType: ["Katar"],
  },
  {
    name: "Lion Knight Badge",
    type: ["Accessory"],
    tier: 2,
    location: "Rare Gear",
    rare: true,
    expansion: "Lion Knight",
  },
  {
    name: "Sleeping Virus Flower",
    type: ["Accessory"],
    tier: 2,
    location: "Rare Gear",
    rare: true,
    expansion: "Flower Knight",
  },
  {
    name: "Replica Flower Sword",
    type: ["Weapon"],
    tier: 2,
    location: "Rare Gear",
    rare: true,
    expansion: "Flower Knight",
    weaponType: ["Grand"],
  },
  {
    name: "Flower Knight Helm",
    type: ["Blacklist"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Flower Knight",
  },
  {
    name: "Trash Crown",
    type: ["Accessory", "Support"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Spidicules",
  },
  {
    name: "Hidden Crimson Jewel",
    type: ["Accessory"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Spidicules",
  },
  {
    name: "Grinning Visage",
    type: ["Tank"],
    tier: 2,
    location: "Rare Gear",
    rare: true,
    expansion: "Spidicules",
  },
  {
    name: "The Weaver",
    type: ["Weapon"],
    tier: 2,
    location: "Rare Gear",
    rare: true,
    expansion: "Spidicules",
    weaponType: ["Sword"],
  },
  {
    name: "Reverberating Lantern",
    type: ["Accessory"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Manhunter",
  },
  {
    name: "Hunter's Heart",
    type: ["Blacklist"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Manhunter",
  },
  {
    name: "Tool Belt",
    type: ["Accessory"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Manhunter",
  },
  {
    name: "Mahunters Hat",
    type: ["Accessory"],
    tier: 2,
    location: "Rare Gear",
    rare: true,
    expansion: "Manhunter",
  },
  {
    name: "Deathpact",
    type: ["Accessory"],
    tier: 4,
    location: "Rare Gear",
    rare: true,
    expansion: "Manhunter",
  },
  {
    name: "God's String",
    type: ["Accessory"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Sunstalker",
  },
  {
    name: "Eye Patch",
    type: ["Accessory"],
    tier: 2,
    location: "Rare Gear",
    rare: true,
    expansion: "Sunstalker",
  },
  {
    name: "Green Armor",
    type: ["Armour"],
    tier: 5,
    location: "Rare Gear",
    rare: true,
    expansion: "Green Armor",
  },
  {
    name: "Fetosaurus",
    type: ["Tank"],
    tier: 5,
    location: "Rare Gear",
    rare: true,
    expansion: "Green Armor",
  },
  {
    name: "Griswaldo",
    type: ["Weapon"],
    tier: 5,
    location: "Rare Gear",
    rare: true,
    expansion: "Green Armor",
    weaponType: ["Sword"],
  },
  {
    name: "Dormant Twilight Cloak",
    type: ["Accessory"],
    tier: 2,
    location: "Rare Gear",
    rare: true,
    expansion: "Twilight Knight",
  },
  {
    name: "Blue Lantern",
    type: ["Accessory"],
    tier: 2,
    location: "Rare Gear",
    rare: true,
    expansion: "Twilight Knight",
  },
  {
    name: "Bloodskin",
    type: ["Accessory"],
    tier: 2,
    location: "Rare Gear",
    rare: true,
    expansion: "White Speaker",
  },
  {
    name: "Speaker Cult Knife",
    type: ["Weapon"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "White Speaker",
    weaponType: ["Dagger"],
  },
  {
    name: "Leather Brassiere (With Lantern Armor)",
    type: ["Armour"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Dark Seamstress",
  },
  {
    name: "Black Knight Badge",
    type: ["Accessory"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Black Knight",
  },
  {
    name: "Black Gauntlet",
    type: ["Accessory"],
    tier: 2,
    location: "Rare Gear",
    rare: true,
    expansion: "Black Knight",
  },
  {
    name: "Iron Hand",
    type: ["Weapon"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Black Knight",
    weaponType: ["Grand"],
  },
  {
    name: "Worn Strafing Bow",
    type: ["Weapon"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Red Witches",
    weaponType: ["Bow"],
  },
  {
    name: "Worn Seeking Bow",
    type: ["Weapon"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Red Witches",
    weaponType: ["Bow"],
  },
  {
    name: "Worn Brawling Bow",
    type: ["Weapon"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Red Witches",
    weaponType: ["Bow", "Katar"],
  },
  {
    name: "Portable Waterphone",
    type: ["Accessory"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Black Knight",
  },
  {
    name: "Shrieking Bow",
    type: ["Weapon"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Black Knight",
    weaponType: ["Bow"],
  },
  {
    name: "Wrath",
    type: ["Weapon"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Black Knight",
    weaponType: ["Sword"],
  },
  {
    name: "Magnum Knives",
    type: ["Weapon"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Black Knight",
    weaponType: ["Dagger"],
  },
  {
    name: "Fellbrandt",
    type: ["Weapon"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Black Knight",
    weaponType: ["Grand"],
  },
  {
    name: "Everburn",
    type: ["Accessory"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Black Knight",
  },
  {
    name: "Camping Bag",
    type: ["Accessory"],
    tier: 3,
    location: "Rare Gear",
    rare: true,
    expansion: "Black Knight",
  },
  {
    name: "Regal Armor",
    type: ["Blacklist"],
    tier: 2,
    location: "Rare Gear",
    rare: true,
    expansion: "Community Edition",
  },
];
